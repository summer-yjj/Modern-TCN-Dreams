...
class Block(nn.Module):
    def __init__(self, large_size, small_size, dmodel, dff, nvars, small_kernel_merged=False, drop=0.1):
        ...
        self.dw = ReparamLargeKernelConv(
            ...
            stride=1,
            groups=in_ch,  # groups 与通道数相同，实现逐通道 depthwise 卷积
...
class ModernTCN(nn.Module):
    def __init__(self,task_name,patch_size,patch_stride, stem_ratio, downsample_ratio, ffn_ratio, num_blocks, large_size, small_size, dims, dw_dims,
        ...
        # head (forecasting head kept for original tasks; classification/segmentation define their own heads)
        patch_num = seq_len // patch_stride
        self.n_vars = c_in  # number of variables / channels
        ...
        # whole-sequence classification head
        if self.task_name == 'classification':
            self.act_class = F.gelu
            self.class_dropout = nn.Dropout(self.class_drop)
            # flatten all variables and temporal features into one vector per sample
            self.head_class = nn.Linear(self.n_vars * self.head_nf, self.class_num)

        # point-wise segmentation head (e.g. DREAMS spindles)
        if self.task_name == 'point_segmentation':
            self.act_seg = F.gelu
            self.seg_dropout = nn.Dropout(self.class_drop)
            # for point segmentation we use the last-stage feature dim per variable
            self.seg_in_dim = dims[self.num_stage - 1] * self.n_vars  # M*D
            self.head_seg = nn.Linear(self.seg_in_dim, self.class_num)
    ...
    def point_segmentation(self, x):
        """
        Point-wise segmentation head.
        输入 x: [B, M, L] （在 Model.forward 里已从 [B, T, C] permute 而来）
        输出 logits: [B, L, num_class]
        """
        B, M, L = x.shape
        feats = self.forward_feature(x, te=None)  # [B, M, D, N]
        B, M, D, N = feats.shape

        feats = self.act_seg(feats)
        feats = self.seg_dropout(feats)

        # [B, M, D, N] -> [B, N, M*D]
        feats = feats.permute(0, 3, 1, 2).reshape(B, N, M * D)
        # apply linear head at each coarse temporal position: [B, N, num_class]
        logits_coarse = self.head_seg(feats)

        # upsample along temporal dimension back to original length L
        # logits_coarse: [B, N, C] -> [B, C, N]
        logits_coarse = logits_coarse.permute(0, 2, 1)
        logits_upsampled = F.interpolate(logits_coarse, size=L, mode='nearest')
        # [B, C, L] -> [B, L, C]
        logits = logits_upsampled.permute(0, 2, 1)
        return logits
    ...
    def forward(self, x, te=None):
        ...
        elif self.task_name == 'point_segmentation':
            x = self.point_segmentation(x)
